        ...
        public override void Update(float deltaTime)
        {
            if (!inventoryMenu.activated)
            {
                ...
                if (!worldCamera.turning)
                {
                    ...
                    if (rayTrace.hit)
                    {
                        bool inRange = (rayTrace.hitPosition - player.worldPosition).sqrMagnitude < 25f || !(player.pickedItem is ISO.Items.ItemBlock || player.pickedItem is ISO.Items.ItemPickaxe);

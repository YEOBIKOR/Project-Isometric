...
public class Player : EntityCreature
{
    ...
    {
        get
        { return pickedItemContainer.item; }
    }
    ...
    public Player() : base(0.3f, 2.0f, 100f)
    {
        ...
        inventory[0].SetItem(new ItemPickaxe());
    ...
    public void AcquireItem(Item item)
    {
        ...
        for (int i = 0; i < inventorySize; i++)
        {
            if (inventory[i].blank)
                returnItem = inventory[i].SetItem(returnItem);

            else if (item.itemID == inventory[i].item.itemID)
                returnItem = inventory[i].SetItem(returnItem);
    ...
    public void DropItem(ItemContainer itemContainer)
    {
        ...
        {
            DroppedItem droppedItem = new DroppedItem(itemContainer.item);
    ...
    public void UseItem(RayTrace rayTrace, bool clicked)
    {
        if (pickedItem != null)
        {
            if (!(itemUseCoolTime > 0f) && (pickedItem.repeatableUse || clicked))
            {
                pickedItem.OnUseItem(this, rayTrace);
                itemUseCoolTime = pickedItem.useCoolTime;
            }
        }
        ...
        public class HoldItem : AnimationPart
        {
            ...
            public override void Update(float deltaTime)
            {
                if (playerGraphics.player.pickedItemContainer.blank)
                    ...
                    item.element = playerGraphics.player.pickedItem.element;
